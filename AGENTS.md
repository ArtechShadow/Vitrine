...
This is the **Vitrine** project — a standalone capture-adaptive video → structured-3D-scene → Unreal Engine 5.8 game-asset pipeline. LichtFeld Studio is a **vendored tool** (pinned at `vendor/lichtfeld-studio` @ v0.5.3) that provides native 3DGS training, rendering, and the MCP server described below. You do not modify the vendored tool; you drive it through its MCP surface.

This repo exposes a large local MCP surface for the vendored LichtFeld Studio tool. When the task is to drive the LichtFeld app, inspect runtime state, manipulate the GUI, run training, edit selections, or export data, prefer MCP discovery before reading source code.
...
See `docs/docs/development/mcp/` for the MCP guide and workflow recipes. For the vendored tool boundary policy, see `BOUNDARIES.md`.
