...
| 14 | ttyd / VNC have a second factor beyond the tunnel | **PARTIAL (plumbed 2026-07-09 pm)** | `VITRINE_TTYD_CREDENTIAL` (ttyd basic-auth; wired in the mounted `vitrine-terminal.sh`, active once the rebuilt image's gated conf is in use) and `VITRINE_VNC_PASSWORD` (mounted `entrypoint.sh`, active on next container restart) added + plumbed through compose. Default remains tunnel-only (warned at boot). Closes the action items under #5/#6 at the config level; operators must set values. |

## Next actions (priority order)

1. **#8 least-privilege** — add `user=` to the ComfyUI supervisord program, create per-component OS users + venvs (ADR-022 D2), move `HF_TOKEN`/`ANTHROPIC_API_KEY` to Docker secrets. *Requires runtime validation on the rig — file-permission regressions are likely.*
2. **Image rebuild** — the baked `supervisord.conf` predates the ttyd gate (#3): the terminal currently RUNS ungated in-container (loopback-published only). Rebuilding the image activates the gate + the `VITRINE_TTYD_CREDENTIAL` path (#14).
