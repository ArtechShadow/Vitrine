...
artifact_vlm_quant  = "Q5_K_M"            # staged gemma-4 vision fallback quant; the local text
                                          # reasoner is host-served DiffusionGemma 26B-A4B Q8_0
                                          # (text-only, :8084), not selected here
...
The local **text reasoner/overseer** is host-served **DiffusionGemma 26B-A4B Q8_0 (~27 GB, text-only)**
on `:8084` — it is not part of this onboarding model selection (it is an external host service, see
Step 5). The table below covers only the per-frame **visual** artifact triage path, which defaults to
`claude_code` because DiffusionGemma cannot see; the staged gemma-4 vision GGUF is the optional fallback.

| GPU VRAM | Inpaint | Hull | Artifact VLM (visual triage) |
|----------|---------|------|-------------|
| 48 GB (A6000) | FLUX.2-dev fp8mixed | Hunyuan3D-2.1 | claude_code (default) / gemma-4 vision fallback Q5_K_M (~20 GB) |
| 24 GB | FLUX.1-Fill fallback | Hunyuan3D-2.0 | claude_code (default) / gemma-4 vision fallback + hard unload |
...
3. Brings up `v2g-net` and verifies the host **DiffusionGemma** reasoner endpoint (`agent-llm`, host `:8084`, OpenAI-compatible, text-only). DiffusionGemma is a host service, not a `v2g-net` container the onboarding starts; visual artifact triage uses `claude_code` (the staged gemma-4 vision GGUF is the optional fallback).
