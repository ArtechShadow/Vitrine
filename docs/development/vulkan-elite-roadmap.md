...
> **Scope note:** This roadmap tracks improvements to the **LichtFeld Studio** Vulkan renderer,
> which Vitrine consumes as a **vendored tool** at `vendor/lichtfeld-studio` (pinned tag v0.5.3).
> Work here is tracked for awareness and potential upstream contribution, but Vitrine's own pipeline
> code does not depend on any specific Vulkan phase landing.
...
Smoke test gate re-run after every phase (run from `vendor/lichtfeld-studio/`):
```
...
Reproduce locally (from `vendor/lichtfeld-studio/`):
```
