...
ADR-013 replaces the hardcoded ``localhost:port`` / ``localhost:port``
...
The legacy ``localhost`` IPs are retained ONLY as documented fallback
...
#: Local agent LLM — DiffusionGemma 26B-A4B (Gemma-4 MoE) reasoner/overseer
#: (D-013.5). Unlike the other services it runs on the GPU *host* (the llama.cpp
#: diffusion-gemma server), not a v2g-net container, so the default is the host
#: LAN address; override with V2G_AGENT_LLM_URL (e.g. http://host.docker.internal:8084
#: when the container has an extra_hosts host-gateway entry). TEXT-ONLY build.
DEFAULT_AGENT_LLM_URL = "http://localhost:8084"
...
ENV_VARS: dict[str, str] = {
    ...
    "agent_llm_url": "V2G_AGENT_LLM_URL",
...
class Endpoints:
    ...
    agent_llm_url: str = DEFAULT_AGENT_LLM_URL
    ...
    def resolve(self, service: str, override: str | None = None) -> str:
        ...
        (``comfyui_url``, ``comfyui_api_url``, ``agent_llm_url``, ``milo_url``,
        ``come_url``) or a bare alias (``comfyui``, ``agent-llm``/``agent_llm``,
        ``llm``, the deprecated ``agent-vlm``/``vlm``, ``milo``, ``come``). A
        non-empty ``override`` always wins. Raises
...
_ALIASES: dict[str, str] = {
    ...
    "agent-llm": "agent_llm_url",
    "agent_llm": "agent_llm_url",
    "llm": "agent_llm_url",
    # Deprecated aliases (pre-DiffusionGemma "agent-vlm" naming) — kept so old
    # manifests / env still resolve to the agent LLM endpoint.
    "agent-vlm": "agent_llm_url",
    "agent_vlm": "agent_llm_url",
    "vlm": "agent_llm_url",
    "milo": "milo_url",
    "come": "come_url",
}
