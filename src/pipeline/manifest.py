...
class PipelineOverrides:
    ...
    agent_llm_url: Optional[str] = None      # -> endpoints.agent_llm_url (DiffusionGemma)


@dataclass
class Oversight:
    backend: str = "claude_code"          # claude_code (default) | diffusiongemma
    artifact_vlm: str = "claude_code"     # claude_code (vision) | diffusiongemma (text-only)
...
class Manifest:
    ...
    def to_pipeline_config(self, base: Optional[PipelineConfig] = None) -> PipelineConfig:
        ...
        Mappings (additive — only set fields the manifest actually declares):
          ...
          * [pipeline].agent_llm_url   -> endpoints.agent_llm_url    (optional override)
        ...
        if self.pipeline.agent_llm_url:
            cfg.endpoints.agent_llm_url = self.pipeline.agent_llm_url
...
def _parse(raw: dict[str, Any]) -> Manifest:
    ...
    pipeline = PipelineOverrides(
        ...
        # Accept the deprecated ``agent_vlm_url`` key (pre-DiffusionGemma) as a
        # fallback so older exhibit.toml files keep working.
        agent_llm_url=(str(pl_raw.get("agent_llm_url") or pl_raw.get("agent_vlm_url"))
                       if (pl_raw.get("agent_llm_url") or pl_raw.get("agent_vlm_url")) else None),
    )

    ov_raw = _as_dict(raw.get("oversight", {}), "oversight")
    # Migrate the legacy ``gemma_local`` value to ``diffusiongemma``.
    def _ov(value: str) -> str:
        return "diffusiongemma" if value == "gemma_local" else value
    oversight = Oversight(
        backend=_ov(str(ov_raw.get("backend", "claude_code"))),
        artifact_vlm=_ov(str(ov_raw.get("artifact_vlm", "claude_code"))),
    )
