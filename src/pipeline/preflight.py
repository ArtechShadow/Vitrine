...
def check_all() -> dict:
    ...
    # Agent LLM (DiffusionGemma) connectivity probe — advisory. The LLM is only
    # needed for the oversight/recovery path, not the core reconstruct->decompose
    # flow, so an absent endpoint is a WARN, never a hard failure.
    llm = check_agent_llm_endpoint()
    if llm is not None:
        summary["agent_llm"] = "ok" if llm.get("smoke_ok") else (
            "reachable" if llm.get("healthy") else "down")

    logger.info("Preflight check PASSED: %s", summary)
    return summary


def check_agent_llm_endpoint() -> dict | None:
    """Probe the local agent LLM (DiffusionGemma) endpoint — advisory.

    Delegates to ``agent_llm.check_agent_llm``: resolves the endpoint, checks
    ``/health`` and (when healthy) runs a 1-block deterministic smoke chat. Logs
    PASS/WARN and returns the structured result, or ``None`` if the client cannot
    be imported (must never break preflight).
    """
    try:
        from pipeline.agent_llm import check_agent_llm
    except Exception as e:  # noqa: BLE001 — optional; never break preflight
        logger.warning("agent_llm client unavailable, skipping LLM probe: %s", e)
        return None

    rep = check_agent_llm()
    if rep.get("smoke_ok"):
        logger.info("Agent LLM PASS: %s (%s) reachable + smoke OK",
                    rep.get("model"), rep.get("endpoint"))
    elif rep.get("healthy"):
        logger.warning("Agent LLM reachable but smoke chat failed: %s (%s) err=%s",
                       rep.get("model"), rep.get("endpoint"), rep.get("error"))
    else:
        logger.warning("Agent LLM DOWN at %s — oversight/recovery path disabled "
                       "(core pipeline unaffected). err=%s",
                       rep.get("endpoint"), rep.get("error"))
    return rep
...
def print_report() -> None:
    ...
    # Agent LLM (DiffusionGemma) connectivity.
    try:
        from pipeline.agent_llm import check_agent_llm

        rep = check_agent_llm()
        state = "PASS" if rep.get("smoke_ok") else (
            "REACHABLE" if rep.get("healthy") else "DOWN")
        print(f"  Agent LLM [{state}] {rep.get('model')} @ {rep.get('endpoint')}"
              + (f"  err={rep.get('error')}" if rep.get("error") else ""))
    except Exception as e:  # noqa: BLE001 — advisory only
        print(f"  (Agent LLM probe skipped: {e})")
