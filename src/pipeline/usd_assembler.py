...
class ObjectDescriptor:
    ...
    hull_glb_path: Optional[str] = None
    ...
    hull_backend: Optional[str] = None
    hull_vertices: int = 0
    hull_faces: int = 0
    view_synth_views: List[str] = field(default_factory=list)
...
class UsdSceneAssembler:
    ...
    def _place_objects(self, stage: Usd.Stage) -> None:
        ...
        for idx, obj in enumerate(self._objects):
            ...
            # v2g:* lineage metadata (preserved by UE Stage Actor + pxr)
            if obj.hull_glb_path:
                prim.SetCustomDataByKey("v2g:hull_glb", obj.hull_glb_path)
            if obj.hull_backend:
                prim.SetCustomDataByKey("v2g:hull_backend", obj.hull_backend)
            if obj.hull_vertices > 0:
                prim.SetCustomDataByKey("v2g:hull_vertices", obj.hull_vertices)
                prim.SetCustomDataByKey("v2g:hull_faces", obj.hull_faces)
            if obj.view_synth_views:
                prim.SetCustomDataByKey("v2g:view_synth", True)
                prim.SetCustomDataByKey(
                    "v2g:view_synth_views", ",".join(obj.view_synth_views),
                )
            prim.SetCustomDataByKey("v2g:confidence", obj.confidence)

            # Per-object custom metadata (caller-supplied)
    ...
    def _write_scene_metadata(self, stage: Usd.Stage) -> None:
        ...
        Both ``v2g:*`` (the Vitrine lineage namespace, preserved through UE
        Stage Actor + pxr) and ``lichtfeld:*`` (upstream compat) are written.
        """
        world = stage.GetPrimAtPath("/World")
        if not world.IsValid():
            return
        world.SetCustomDataByKey("v2g:pipeline_version", "1.0")
        world.SetCustomDataByKey("v2g:up_axis", self._up_axis)
        world.SetCustomDataByKey("v2g:meters_per_unit", self._meters_per_unit)
        world.SetCustomDataByKey("v2g:object_count", len(self._objects))
        # lichtfeld:* mirror for backward compatibility
        ...
        for key, value in self._scene_metadata.items():
            world.SetCustomDataByKey(f"v2g:{key}", value)
