...
class RenderConfig:
    ...
    # Outlier-robust centring/scaling: ignore the extreme `robust_pct`% tails per
    # axis when computing the object's centre + extent (segmented per-object
    # splats carry stray Gaussians that otherwise collapse the object to 0% cov).
    # 0 = raw min/max (legacy).
    robust_pct: float = 2.0
    antialias: bool = True
...
def generate_orbit_cameras(
    ...
    elif config.azimuth_preset == "trellis_6":
        # TRELLIS.2 multi-view: the full-360 panel set mapped to the node's named
        # slots (front/back/left/right/top/bottom). Front/left/back/right at 0
        # elevation give the equatorial band; a top view (looking down) and a
        # bottom view (looking up) cap the object so its top face and underside
        # are reconstructed, not just the sides. This is the "multi-panel 360
        # turnaround" feed — every TRELLIS view slot filled from the splat.
        azimuths = [0.0, 90.0, 180.0, 270.0, 0.0, 0.0]
        elevations = [0.0, 0.0, 0.0, 0.0, 85.0, -85.0]
...
class GaussianData:
    ...
    def robust_bounds(self, pct: float = 2.0) -> tuple[np.ndarray, float]:
        """Outlier-robust (center, extent) via per-axis percentiles.

        Segmented per-object splats routinely carry a few stray Gaussians far
        from the object (segmentation leakage). Those blow up the raw min/max
        bbox so ``scale_to_unit`` shrinks the real object to sub-pixel and every
        rendered panel comes back empty (0% coverage). Using the ``pct``..``100-pct``
        percentile bbox ignores the extreme tails so the object is centred and
        scaled correctly. ``pct=0`` reproduces the raw min/max behaviour."""
        if pct <= 0:
            return self.center.copy(), self.extent
        lo = np.percentile(self.positions, pct, axis=0)
        hi = np.percentile(self.positions, 100.0 - pct, axis=0)
        center = (lo + hi) / 2.0
        extent = float(np.linalg.norm(hi - lo))
        # Degenerate guard: fall back to raw bounds if the percentile band collapsed.
        return (center, extent) if extent > 1e-9 else (self.center.copy(), self.extent)
...
class MultiViewRenderer:
    ...
    def render(
        ...
        # Optionally center and scale (outlier-robust — see robust_bounds).
        r_center, r_extent = gaussians.robust_bounds(self.config.robust_pct)
        if self.config.center_object:
            gaussians.positions = gaussians.positions - r_center
            logger.info("Centered object (robust shift by %s)", r_center)

        if self.config.scale_to_unit:
            if r_extent > 0:
                scale_factor = 2.0 / r_extent
                gaussians.positions *= scale_factor
                gaussians.scales += math.log(scale_factor)
                logger.info("Scaled to unit sphere (robust extent %.3f, factor %.4f)",
                            r_extent, scale_factor)
