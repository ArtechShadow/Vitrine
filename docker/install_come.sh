...
# CoMe's train.py does `from fused_ssim import fused_ssim`, but decoupled-fused-ssim's
# setup.py mis-declares packages=['fused_ssim'] while shipping the dir as
# decoupled_fused_ssim/ → the `fused_ssim` import name never lands. Alias it.
COME_SP="$($RUN_COME python -c 'import decoupled_fused_ssim,os;print(os.path.dirname(os.path.dirname(decoupled_fused_ssim.__file__)))')"
ln -sfn "$COME_SP/decoupled_fused_ssim" "$COME_SP/fused_ssim" && echo "aliased fused_ssim -> decoupled_fused_ssim"
...
# NB: triangulation.cpp/py_binding.cpp #include <cuda_runtime.h> as plain C++ (not nvcc),
# so the CUDA include dir must be on the C++ compiler path → export CPATH (else "cuda_runtime.h:
# No such file"). Also DCMAKE_POLICY_VERSION_MINIMUM=3.5 for the old CMakeLists.
( cd submodules/tetra-triangulation \
    && export CUDA_HOME=/usr/local/cuda CPATH=/usr/local/cuda/include CPLUS_INCLUDE_PATH=/usr/local/cuda/include \
    && $RUN_COME cmake . -DCMAKE_PREFIX_PATH="${CONDA_ENV_PREFIX}" -DCMAKE_POLICY_VERSION_MINIMUM=3.5 \
    && $RUN_COME bash -c "CPATH=/usr/local/cuda/include make -j$(nproc)" \
