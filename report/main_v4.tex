...
generators (TRELLIS.2 / Hunyuan3D-2.1); the Hunyuan3D-2.1 ComfyUI node now imports (the container was
fixed, 2026-06-08), but the hull weights are not pulled, so meshing stays on the TSDF fallback. The local
agent LLM \emph{is} now wired --- \code{DiffusionGemma-26B-A4B} runs host-served (OpenAI API on \code{:8084})
and the pipeline reaches it from inside the container --- but it is \emph{text-only}, so the generative
\emph{recovery} of unseen object faces (FLUX.2) and \emph{visual} artefact triage (which needs the staged
gemma-4 vision model) remain staged-but-not-wired. Those are reported as \wip{} or \designed{} below, not as
done. Quality, too,
...
\midrule
Local agent LLM (DiffusionGemma) & \validated{} & \code{DiffusionGemma-26B-A4B} host-served (OpenAI API \code{:8084}); \code{agent\_llm.py} client + preflight probe reach it from inside the container (\code{Agent LLM [PASS]}). Text-only. \\
SOTA single-image hulls (TRELLIS.2 / Hunyuan3D-2.1) & \wip{} & ComfyUI fixed; Hunyuan3D-2.1 node now imports. Hull weights not pulled and TRELLIS2 needs a compiled \code{comfy\_sparse\_attn} ext, so meshing stays on gsplat-TSDF. \\
FLUX.2 generative recovery loop & \wip{} & Client built; not yet wired into the per-object hull path (unseen-face inpaint). \\
Visual artefact triage (gemma-4 vision) & \wip{} & DiffusionGemma is text-only; visual triage defaults to \code{claude\_code} or the staged gemma-4 vision GGUF, not yet served. \\
...
013 & \code{exhibit.toml} manifest; serial lifecycle; \code{v2g-net}; DiffusionGemma agent LLM & \validated{} \\
...
\begin{enumerate}
  ...
  \item \textbf{SOTA hulls:} pull the TRELLIS.2 / Hunyuan3D-2.1 hull weights and build the TRELLIS2
        \code{comfy\_sparse\_attn} extension (the Hunyuan3D-2.1 node already imports), replacing the
        gsplat-TSDF fallback for key objects.
  \item \textbf{Recovery:} wire the FLUX.2 unseen-face inpaint loop. The DiffusionGemma agent LLM is wired
        as the text reasoner/overseer; for the \emph{visual} verify-or-retry judge, serve the staged gemma-4
        vision model (DiffusionGemma is text-only).
