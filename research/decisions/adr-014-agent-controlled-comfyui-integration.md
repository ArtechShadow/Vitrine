...
## Amendment (2026-06-08) — text-only agent LLM; visual evaluate needs the vision fallback

The `RecoveryController` "evaluate / looks-at" step (D-014.3) assumed the unified gemma-4 vision agent.
Since the agent LLM is now **text-only DiffusionGemma-26B-A4B** (host-served :8084, OpenAI-compatible;
see ADR-013 amendment 2026-06-08), the *visual* evaluate step requires the staged gemma-4 vision
fallback model (or the `claude_code` backend); DiffusionGemma handles only the planning/reasoning around
it. ComfyUI is reached via `vitrine-comfyui:8188` (`v2g-net`) / host `:8200`, and the custom-node
dependencies were fixed — see `scripts/comfyui_entrypoint.sh`.
...
- **A real ComfyUI client** — `src/pipeline/comfyui_inpainter.py` (`ComfyUIInpainter`). It already
  ...
  - **Salad control-plane API** (`api_url`, default `http://localhost:3001`) — the owner confirms
  ...
  - **Direct ComfyUI graph API** (`comfyui_url`, default `http://localhost:8189`) — `object_info`
...
1. **The client is hardcoded to `localhost:{3001,8189}`** (`__init__` defaults, lines 282-284) and
...
- Replace the hardcoded `localhost:{3001,8189,8188}` defaults with **service-DNS endpoints**
