...
All weights are in-repo in the single unified model tree at `data/comfyui/models`
(mounted in-container at `/comfyui/models`, and at `/models-staging:ro` for
gaussian-toolkit):

| Asset | Path | Use |
|-------|------|-----|
| FLUX.2-dev | `data/comfyui/models/diffusion_models/flux2_dev_fp8mixed.safetensors` | inpaint upgrade (item 1) |
| FLUX.2 VAE | `data/comfyui/models/vae/flux2-vae.safetensors` | inpaint |
| FLUX.2 text encoder | `data/comfyui/models/text_encoders/mistral_3_small_flux2_{fp8,bf16}.safetensors` | inpaint |
| FLUX.2 Turbo LoRAs | `data/comfyui/models/loras/Flux2TurboComfyv2.safetensors`, `Flux_2-Turbo-LoRA_comfyui.safetensors` | speed |
| Hunyuan3D 2.1 | `data/comfyui/comfy/ldm/hunyuan3dv2_1` | hull upgrade (item 2) |
| Hunyuan3D 2 MV | `data/comfyui/models/hunyuan3d/hunyuan3d-dit-v2-mv` | hull fallback |
| SAM3D node + weights | `data/comfyui/custom_nodes/comfyui-sam3dobjects`, `data/comfyui/models/sam3d/` | hull fallback (item 3) |
| Local ComfyUI | `data/comfyui` (a full install) | dev test target |

**Weights are in-repo** at `data/comfyui/models` — ComfyUI's native model store
(`/comfyui/models` in-container); nothing to mount or bootstrap.
...
- Hardcoded `localhost:8189`(ComfyUI)/`:3001`(Salad) in ~11 sites:
