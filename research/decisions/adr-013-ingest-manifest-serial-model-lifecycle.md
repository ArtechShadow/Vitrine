...
Proposed — Open Questions resolved 2026-06-04. **Target host: localhost** (develop scaffolding on
...
## Amendment (2026-06-08) — local model is host-served DiffusionGemma (text-only)

D-013.5's local model is now **DiffusionGemma-26B-A4B** (Gemma-4 MoE, ~4B active, **Q8_0**), served on
the GPU **host** by the llama.cpp `llama-diffusion-gemma-visual-server` (OpenAI-compatible, **:8084**,
model id `diffusiongemma-26B-A4B-it-Q8_0`) — not a containerised `agent-vlm` gemma-4. The new client is
`src/pipeline/agent_llm.py` (`AgentLLM`); the service field/alias is renamed `agent_vlm_url`→`agent_llm_url`
(default `http://localhost:8084`, in-container override `V2G_AGENT_LLM_URL=http://host.docker.internal:8084`;
deprecated `agent-vlm`/`vlm` aliases still resolve). This GGUF build is **text-only**, so the artifact-VLM
(FR-27 visual triage) role now defaults to `claude_code` while DiffusionGemma fills the FR-28 text
reasoner/overseer role (a staged gemma-4 vision GGUF + mmproj is the registry fallback). The `gemma_local`
oversight value is renamed `diffusiongemma`; `preflight.py` runs an advisory connectivity probe at startup.
This supersedes the gemma-4 `agent-vlm` decision; the oversight architecture is otherwise unchanged.
...
2. **No model lifecycle.** Every generative endpoint is a long-lived service hardcoded to a single
   host (`config.py:135-136,152-153` → `http://localhost:{3001,8189}`; `:171` → `localhost:8188`).
...
3. **No threading topology.** The hardcoded `localhost:port` endpoints couple the pipeline to one
...
- **FLUX.2-dev is real and already staged.** `workspace/ai-3d-fusion/workflows/glass-medallion-flux2-dualgpu.json`
  ...
  `http://localhost:8188` (ComfyUI 0.3.75). This is the owner's "flux2 dev I think we downloaded" —
  confirmed, but it lives on the **localhost GPU workstation**, which this `agentbox` container
  cannot route to.
...
Replace hardcoded `localhost:port` / `localhost:port` endpoints with a user-defined bridge network
...
- **Q1 — Host & VRAM. RESOLVED.** Target deployment is **localhost**; the pipeline moves there once
