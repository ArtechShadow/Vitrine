...
Weights live in the single unified in-repo model tree `data/comfyui/models`
(~216 GB, gitignored — ComfyUI's native model store). The compose stack bind-mounts
it directly; no copy/bootstrap step:
```yaml
volumes:
  # vitrine-comfyui: ComfyUI runtime + native model store
  - ./data/comfyui:/comfyui            # models native at /comfyui/models
  # gaussian-toolkit: read-only access to the same model tree
  - ./data/comfyui/models:/models-staging:ro
```
...
```bash
cd /home/john/githubs/gaussian/LichtFeld-Studio
docker build -t gaussian-toolkit:latest -f docker/Dockerfile .
