...
## Amendment (2026-06-22) — USD is no longer the delivered contract; `v2g:*` lineage rides on UE asset tags (ADR-019)

This ADR's `v2g:*` customData **schema** (the §(a) fields, §(b) population, §(d)
lineage threading) is unchanged and remains correct. What changes is the **carrier
of record for delivery**: this ADR assumed the **self-describing USD scene graph is
the delivered contract** (Consequences: "The USD is self-describing"; the rejected
"side-car the metadata in an external JSON" alternative). **ADR-019 demotes delivered
USD**: the UE end-state consumes **textured-mesh game assets (FBX), not USD** —
because LichtFeld's native USD emits an un-importable `ParticleField`/gaussian prim,
because UE drops USD `customData`/`displayColor` (and GLB `COLOR_0`) on import, and
because captured color survives only as a baked UV texture. Consequently:

- **The `v2g:*` lineage that this ADR authors as USD customData is, for the UE
  delivery path, mirrored onto UE asset metadata tags**
  (`unreal.EditorAssetLibrary.set_metadata_tag`) at import — not delivered inside a
  USD the UE stage reads. The schema (field names, required/optional, sources) is
  identical; only the delivered carrier moves from USD customData → UE asset tags.
- **USD remains valid as an optional / archival side-artifact** still carrying the
  full `v2g:*` customData per this ADR (useful for inspection, archival, and
  non-UE consumers). It is just no longer *the* delivered self-description contract
  for the UE deliverable.

The §(c) "real textured environment mesh prim" decision is also recontextualised: the
textured **mesh** is still the env surface, but it is delivered to UE as a baked-
texture **FBX**, with USD environment-prim authoring retained only for the archival
side-artifact. See `adr-019-mesh-game-assets-not-usd-into-ue.md`.
