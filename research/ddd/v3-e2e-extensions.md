...
2. `has_texture` must be `True` before SceneAssembly consumes it (D11, FR-13, G11).
   No untextured grey hull is delivered. **Refinement (2026-06-21 e2e run, L2/L3):**
   "textured" here means a material the **downstream UE consumer can read** — a baked
   `UsdUVTexture` / textured GLB, or an explicit VertexColor material. A vertex-colour-only
   GLB (`COLOR_0`) is **not** sufficient: UE Interchange GLB import ignores vertex colours
   ("no materials assigned") and UE USD import drops the `displayColor` primvar (flat
   white). The proven bake is Blender Smart UV Project + Cycles DIFFUSE GPU bake
   (`bake_vertex_colors_to_texture()` in `blender_assembler.py`), which is reliable on a
   clean watertight `Hull` (contrast §3.2, where it collapses on the room mesh). The
   "grey hull" failure mode is also a *colour-drop* trap, not only a missing-bake one:
   filtering mesh components without remapping vertex colours through the kept-vertex
   index map yields trimesh's default grey `[102,102,102]` (L3, §3.1).
...
> **Colour-preservation caveat (2026-06-21 e2e run, L3).** The "no grey hull"
> invariant (§2.2) is the same trap as `exhibit_builder` component filtering: when a
> mesh is split into connected components and rebuilt, vertex colours **must** be
> remapped through the kept-vertex index map. Rebuilding a `Trimesh` without colours
> yields trimesh's default grey `[102,102,102]` — the source of a "white/grey in UE"
> red herring this session. Use `trimesh.graph.connected_components` + an explicit
> index remap, **not** `trimesh.split()` + `concatenate()` (which hangs at ~1.8M
> verts). The `MetadataRecord.recon_method` may record a textured hull while the mesh
> itself silently lost its colour at this step — the colour-drop is upstream of, and
> independent of, the metadata block.
...
> **Bake-collapse caveat (2026-06-21 e2e run, L2).** The §2.2 Cycles bake that is
> reliable on a clean watertight `Hull` **collapses on room-scale MILo meshes**: their
> many thin/disconnected components (876 removed on `scene02`) produce degenerate
> near-zero-area UV islands under Smart UV Project, yielding a near-black 2048² atlas.
> The bake therefore is **not** the path to captured environment colour. For
> room/scene-scale colour the highest-fidelity deliverable is a **direct
> Gaussian-splat (gsplat) render**, not a baked env-mesh texture. Consequence for the
> contract: `EnvironmentMesh.has_texture == True` is the target, but where the bake
> collapses the *delivered scene colour* is carried by the gsplat render alongside the
> mesh, and the UE consumer's flat-white result (below, §5.4) is a symptom of the same
> texture-vs-displayColor gap.
...
| **captured colour → UE** (new, L2) | `displayColor` primvar vs. textured material | **Caveat for the downstream UE consumer**: a textured GLB / `displayColor` primvar does **not** survive UE import — UE USD import **drops `displayColor`** (renders flat white) and UE Interchange GLB import ignores vertex colours. The delivered colour therefore depends on a **baked-texture (`UsdUVTexture`) or explicit VertexColor material**, not on `displayColor`. Where the env-mesh bake collapses (§3.2), the captured-colour deliverable for the UE consumer is the **direct gsplat render**, not the env-mesh texture |
...
flowchart TB
    ...
    USD -. "OpenUSD (extended: v2g:* + lineage; UE drops displayColor — needs baked-texture/VertexColor material)" .-> SA
...
> **Colour caveat on the OpenUSD edge (2026-06-21 e2e run, L2/L3).** The USD → SceneAssembly
> edge carries metadata *and* a colour contract for the downstream UE consumer: UE USD
> import **drops the `displayColor` primvar** (flat white) and UE GLB import ignores vertex
> colours, so delivered colour must ride on a **baked-texture (`UsdUVTexture`) or
> VertexColor material**. The §2.2 Cycles bake delivers this on clean watertight `Hull`s
> but collapses on room-scale `EnvironmentMesh`es (§3.2) — for scene colour the
> highest-fidelity deliverable is the **direct gsplat render**, not a baked env-mesh
> texture. The "grey hull" path is additionally a vertex-colour-drop trap during component
> filtering (§3.1, L3).
