...
flowchart TD
    ...
    subgraph P2["Phase 2 · Reconstruction (branching model choices)"]
        direction TB
        POOL["Pool tagged frames for the room/session<br/>(multi-capture: COMBINE several handheld sweeps of the<br/>SAME room → scene02 ~146k pts/95% reg &gt; scene01 90k pts — L6)"]:::done
        SEL["Frame selection · Fibonacci coverage (ADR-007)"]:::done
        MATCH{"SfM branch: COLMAP {GPU SIFT* | ALIKED+LightGlue}<br/>· optional neural SfM {VGGT|MASt3R} (ADR-012)<br/>(*SIFT = working default; ALIKED_LIGHTGLUE invalid enum in<br/>COLMAP 4.1.0 — needs libcudnn.so.9; opts = --FeatureExtraction.*)"}:::branch
        SFM["COLMAP SfM → sparse model"]:::done
        REG{"Registration ≥ 70%?<br/>(GPU SIFT gave 100% reg on real e2e — L4)"}:::part
    ...
    subgraph P4["Phase 4 · Per-key-item 360° hull reconstruction (branching)"]
        direction TB
        OBJ{{"For each KEY item<br/>(scene-best capture ≠ object-best — objects need a<br/>dedicated close orbit, e.g. IMG_0580, for turnaround hulls — L6)"}}:::part
        ...
        HULLTEX["Textured watertight hull + preserved world pose<br/>(captured color → BAKED TEXTURE: Blender Smart UV Project +<br/>Cycles DIFFUSE GPU bake, blender_assembler.py — works on clean<br/>HULLS; UE drops displayColor primvar / ignores COLOR_0 — L2/L8)"]:::part
    end
    ...
    subgraph P5["Phase 5 · Environment mesh (branching backend, ADR-003)"]
        direction TB
        ENV_B{"mesh_method branch:<br/>auto|tsdf|milo*|come|gaussianwrapping<br/>(*MILo radegs GPU default — ADR-012; NOT numpy TSDF — L8)<br/>(ops: MILO_DIR=/opt/milo/milo; undistorted/images/ MUST be<br/>FLAT — MILo refs images by BASENAME, flatten subfolders — L5)"}:::branch
        ENVMESH["Polygonal ENVIRONMENT mesh<br/>(room-scale captured color = DIRECT gsplat splat render, NOT a<br/>baked atlas — Smart UV collapses on messy room meshes — L2/L8)"]:::miss
    end
    ...
    subgraph P6["Phase 6 · USD scene graph (richly annotated)"]
        ...
        MATS["Bind UsdPreviewSurface materials + textures<br/>(UE ingest needs a BAKED-TEXTURE / VertexColor material — UE<br/>USD import drops displayColor, GLB import ignores COLOR_0 — L2;<br/>component-filter MUST remap vertex colors via kept-vertex index<br/>map or hull renders trimesh-gray [102,102,102] — L3)"]:::done
...
flowchart LR
    ...
    A["Frames"]:::branch
    A --> M1{matcher}:::branch
    M1 --> gpu_sift_default & sift_sequential & aliked_lightglue_blocked
    gpu_sift_default --> T{training}:::branch
    sift_sequential --> T
    aliked_lightglue_blocked --> T
    T --> mrnf & mcmc
    ...
    gsplat_TSDF --> EM
    EM --> auto & tsdf & milo_radegs_gpu_default & come & gaussianwrapping
```

> **SOTA defaults (ADR-012):** matcher → **GPU SIFT** is the *working* default — `aliked_lightglue`
> is the aspirational indoor preset but `ALIKED_LIGHTGLUE` is an invalid enum in COLMAP 4.1.0
> (needs `libcudnn.so.9`, missing) so GPU SIFT was the real path at 100% reg (L4); COLMAP 4.1.0 opts
> moved to `--FeatureExtraction.*` / `--FeatureMatching.*`; hull → `Hunyuan3D 2.1`; inpaint →
> `FLUX.2-dev` (ADR-013, amended from Kontext); `mesh_method` default → `milo` (radegs **GPU**, not
> a numpy TSDF — L8; TSDF only on sidecar-down); optional neural SfM (`VGGT`/`MASt3R`) precedes
> COLMAP for low-overlap captures. **Multi-capture (L6):** combine several handheld sweeps of the
> same room for scene coverage; the best video for the *scene* is not the best for an *object*
> (objects need a dedicated close orbit for turnaround hulls).
