...
def test_modelspec_defaults_soft():
    spec = ModelSpec(name="diffusiongemma", engine="llama.cpp",
                     endpoint="http://localhost:8084", vram_estimate_gb=20.0)
...
def test_soft_unload_posts_free(monkeypatch):
    ...
    assert calls["free_url"] == "http://localhost:8084/free"
...
def test_no_gpu_no_docker_no_network_never_raises(monkeypatch):
    ...
    soft = ModelSpec(name="agent_llm", engine="llama.cpp",
                     endpoint="http://localhost:8084", vram_estimate_gb=20.0)
...
def test_endpoints_dns_defaults():
    ...
    assert e.agent_llm_url == "http://localhost:8084"
...
def test_endpoints_resolve_precedence():
    ...
    assert e.resolve("agent-llm") == "http://localhost:8084"
    assert e.resolve("agent-vlm") == "http://localhost:8084"  # deprecated alias
...
def test_legacy_constants_present_but_not_default():
    ...
    assert ep.LEGACY_SINGLE_HOST["comfyui_url"] == "http://localhost:8189"
    assert ep.LEGACY_SINGLE_HOST["comfyui_api_url"] == "http://localhost:3001"
    e = ep.Endpoints.from_env(env={})
    assert "localhost" not in e.comfyui_url
    assert "localhost" not in e.comfyui_api_url
